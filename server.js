...
app.delete('/api/timetable/schedule/:scheduleName', (req, res) => { 
    ...
    schedule.splice(index,1); // go to this index and remove one object 
...
function sendHttpRequest(method, url, data) {
    const promise = new Promise ((resolve, reject)=>{
            ...
            result.onload = () => {
                const resData = JSON.parse(result.response);// coverts JSON into raw js objects and data types
                if(result.status>=400){ // if the status is above 400 then reject it 
                    reject(resData);
                }else{
                     resolve(resData);
                }
                ...
                result.onerror = () => { // error handler, if the request succeeds even if it has a error attached to it. Then overall it would be considered as a success 
                    reject('Something went wrong!'); 
                  };
              
                result.send(JSON.stringify(data)); // converting js to json if we do a post request
    ...
    function displayResults(){ 
        let subjectName = document.getElementById("subjectSearch"); // storing user input into a variable 
        let valSubjectName = subjectName.value.toString().toUpperCase(); // converting the inputted value to string and making it uppercase 
    
    
        let courseCode = document.getElementById("courseSearch"); // storing user input into a variable 
        let valCourseCode = courseCode.value.toString().toUpperCase(); 
      
    
        let component = document.getElementById("componentSearch"); // storing user input into a variable 
        let valComponent = component.value.toString().toUpperCase(); 

        let finalResult = " ";
       
if(valCourseCode === "" && valComponent === ""){// if user only inputs subject 
    sendHttpRequest('GET','http://localhost:5000/api/timetable/subject/'+valSubjectName).then(responseData => { // calling function to execute the htttp request
    console.log(responseData);
    for(i=0;i<responseData.length;i++){
        finalResult = finalResult + "\n" + "Class Name: " + responseData[i].className + "\n" + "Subject: " + responseData[i].subject + "\n" + "Course Code: " + responseData[i].catalog_nbr + "\n" + "Class Name: " + responseData[i].catalog_description + "\n"; // creating a long string to display it to user
    }
    alert(finalResult);
...
else if(valSubjectName === "" && valComponent === ""){ // if user only inputs course
    sendHttpRequest('GET','http://localhost:5000/api/timetable/coursecode/'+valCourseCode).then(responseData => { // calling function to execute the htttp request
    console.log(responseData);
    for(i=0;i<responseData.length;i++){
        finalResult = finalResult + "\n" + "Class Name: " + responseData[i].className + "\n" + "Subject: " + responseData[i].subject + "\n" + "Course Code: " + responseData[i].catalog_nbr + "\n" + "Class Name: " + responseData[i].catalog_description + "\n"; // creating a long string to display it to user
    }
...
else if(valCourseCode === "" && valSubjectName === ""){// if user only inputs component
sendHttpRequest('GET','http://localhost:5000/api/timetable/component/'+valComponent).then(responseData => { // calling function to execute the htttp request
    console.log(responseData);
    for(i=0;i<responseData.length;i++){
        finalResult = finalResult + "\n" + "Class Name: " + responseData[i].className + "\n" + "Subject: " + responseData[i].subject + "\n" + "Course Code: " + responseData[i].catalog_nbr + "\n" + "Class Name: " + responseData[i].catalog_description + "\n"; // creating a long string to display it to user
    }
...
else if(valSubjectName !== "" && valCourseCode!== "" && valComponent === ""){ // if user inputs just course and sbject
    sendHttpRequest('GET','http://localhost:5000/api/timetable/search/subject/coursecode/'+valSubjectName + '/' + valCourseCode ).then(responseData => { // calling function to execute the htttp request
    console.log(responseData);
    for(i=0;i<responseData.length;i++){
        finalResult = finalResult + "\n" + "Class Name: " + responseData[i].className + "\n" + "Subject: " + responseData[i].subject + "\n" + "Course Code: " + responseData[i].catalog_nbr + "\n" + "Class Name: " + responseData[i].catalog_description + "\n"; // creating a long string to display it to user
    }
...
// alert("Here are your results:" + "\n" + responseData[0].className);
  ...
  console.log(err);
    ...
    sendHttpRequest('GET','http://localhost:5000/api/timetable/search/'+valSubjectName + '/' + valCourseCode + '/' + valComponent).then(responseData => { // calling function to execute the htttp request
    console.log(responseData);
    for(i=0;i<responseData.length;i++){
        finalResult = finalResult + "\n" + "Class Name: " + responseData[i].className + "\n" + "Subject: " + responseData[i].subject + "\n" + "Course Code: " + responseData[i].catalog_nbr + "\n" + "Class Name: " + responseData[i].catalog_description + "\n"; // creating a long string to display it to user
    }
...
}; // display Result function ends here

function handleSchedule(){
    var letterNum = /^[0-9a-zA-Z]+$/;  // storing A-Z and a-z in a variable  
   
    let scheduleName = document.getElementById("scheduleName"); // storing user input into a variable 
    let valScheduleName = scheduleName.value.toString();
    if(scheduleName.value.match(letterNum)){
        sendHttpRequest('POST','http://localhost:5000/api/timetable/schedule', {
            "schedule_name": valScheduleName
    
        }).then(responseData => { // calling function to execute the htttp request
        console.log(responseData);
    ...
    // alert("Here are your results:" + "\n" + responseData[0].className);
      ...
      console.log(err);
        ...
        alert("Please input alphabets from A-Z or 0-9 only!");
    }
...
function validateInput(){
    ...
    if(subjectName.value.match(letterNum) && courseCode.value.match(letterNum) && component.value.match(letterNum)){ // only letters and numbers are valid input
        addClasses();
    }else{
        alert("Please input alphabets from A-Z or 0-9 only!");
    }
...
function addClasses(){
    ...
    let subjectName = document.getElementById("subjectSearch"); // storing user input into a variable 
        ...
        let sendData; // declaring an empty object 
    ...
    if(responseData){
            sendData = {
                ...
                "items":[
                    {
                    "subjectCode": responseData[0].subject,
                    "courseCode": responseData[0].catalog_nbr
                   }
                 
                   
                ]
            }
            fetch(`http://localhost:5000/api/timetable/schedule/${valScheduleName}` , { // fetch is used call the backend api and get a response 
                method: 'PUT',
                headers: {
        ...
        alert("No such result found! Please try again!");
    }
